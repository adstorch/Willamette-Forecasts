...
willAge6Fit.dat<-tail(head(data.frame(ratio_65yr = willChsRet.dat$age6_col/willChsRet.dat$age5_col),-4),5)[,1]

## create a vector to generate prediction
willAge6Pred.dat<-tail(head(willChsRet.dat$age5_col,-3),1)
...
willAge6Mod.dat <- list(ratio_65yr=willAge6Fit.dat,
                        age5_col=willAge6Pred.dat,
                        nObs=length(willAge6Fit.dat),
...
{
  list(muRatio_65yr=rnorm(1,0,1),sig=1)
}
...
params.willAge6 <- c("muRatio_65yr",
                     "sig",
                     "pred_willAge6",
                     "q",
                     "r",
...
## call jags
start <- Sys.time()
fit.willAge6 <- jags.parallel(data = willAge6Mod.dat,
...
pred.mcmc.willAge6 <- mcmc.willAge6[, paste("pred_willAge6",sep = "")]

## summarize output for current prediction
### create data frame to store output
willAge6.pred.out<-data.frame(model=character(),
...
willAge6.pred.out[1,]<-c(willAge6Mod.name,
...
willAge6.predHat.out<-data.frame(model=character(),
                                 mean.pred.willAge6=numeric(),
                                 lwrHDI.willAge6=numeric(),
                                 UprHDI.willAge6=numeric(),
                                 stringsAsFactors = FALSE)
...
#### append output to new data frame (from above)
willAge6.predHat.out[1,]<-c(paste(willAge6Mod.name,"hatchery"),
