...
### fit OLS
#### age 3
willAge3Init.mod <- lm(log(willAge3_32yr)~log(willAge2_ret),data=willAge3Fit.dat)
#### age 4
willAge4Init.mod <- lm(log(willAge4_col_mAge4)~log(willAge3_col)+noaa_ranks,data=willAge4Fit.dat)
#### age 5
willAge5Init.mod <- lm(log(willAge5_col_mAge5)~log(willAge4_col_mAge5)+sp_pdo+sp_trans+ichthy_biom+cope_rich,data=willAge5Fit.dat)

### extract coefficient estimates
#### age 3
willAge3Init.beta <- as.numeric(willAge3Init.mod$coef[2])
willAge3Init.betaSE <- coef(summary(willAge3Init.mod))[2,2]
#### age 4
willAge4Init.beta2 <- as.numeric(willAge4Init.mod$coef[3])
willAge4Init.beta2SE <- coef(summary(willAge4Init.mod))[3,2]
#### age 5
...
willCombMod.dat <- list(
  ### age 3
  willAge3_32yr=as.numeric(c(log(willAge3Fit.dat$willAge3_32yr),"NA")),
  willAge2_ret=log(c(willAge3Fit.dat$willAge2_ret,willAge3Pred.dat)),
  nObs.willAge3=length(willAge3Fit.dat$willAge2_ret)+1,
  ### age 4
  willAge4_col_mAge4=as.numeric(c(log(willAge4Fit.dat$willAge4_col),"NA")),
  willAge3_col=log(c(willAge4Fit.dat$willAge3_col,willAge4Pred.dat$age3_col)),
  noaa_ranks=c(willAge4Fit.dat$noaa_ranks,willAge4Pred.dat$noaa_ranks),
  nObs.willAge4=length(willAge4Fit.dat$willAge3_col)+1,
  ### age 5
  willAge5_col_mAge5=as.numeric(c(log(willAge5Fit.dat$willAge5_col),"NA")),
  willAge4_col_mAge5=log(c(willAge5Fit.dat$willAge4_col,willAge5Pred.dat$willAge4_col)),
  ...
  nObs.willAge5=length(willAge5Fit.dat$willAge5_col)+1,
  ### age 6
  ratio_65yr=willAge6Fit.dat,
  willAge5_col_mAge6=willAge6Pred.dat,
  nObs.willAge6=length(willAge6Fit.dat),
  ### hatchery proportion
...
{
  list(beta.willAge3 = runif(1,willAge3Init.beta-(5*willAge3Init.betaSE),willAge3Init.beta+(5*willAge3Init.betaSE)),
       beta2.willAge4 = runif(1,willAge4Init.beta2-(5*willAge4Init.beta2SE),willAge4Init.beta2+(5*willAge4Init.beta2SE)),
       beta1.willAge5 = runif(1,willAge5Init.beta1-(5*willAge5Init.beta1SE),willAge5Init.beta1+(5*willAge5Init.beta1SE)),
...
cat('
  model {
    ...
    for (i in 1:nObs.willAge3){
      ...
      muWillAge3_32yr[i]<-alpha.willAge3[i]+beta.willAge3*(willAge2_ret[i]-mean(willAge2_ret[]))
    ...
    for (j in 1:nObs.willAge4){
      ...
      muWillAge4_col[j]<-alpha.willAge4[j]+beta1.willAge4[j]*(willAge3_col[j]-mean(willAge3_col[]))+beta2.willAge4*(noaa_ranks[j]-mean(noaa_ranks[]))
      ### age return predictions on the arithmetic scale
    ...
    for (k in 1:nObs.willAge5){
      ...
      ### age 4 predictions in log space
      muWillAge5_col[k]<-alpha.willAge5[k]+beta1.willAge5*(willAge4_col_mAge5[k]-mean(willAge4_col_mAge5[]))+beta2.willAge5*(sp_pdo[k]-mean(sp_pdo[]))+beta3.willAge5*(sp_trans[k]-mean(sp_trans[]))+beta4.willAge5*(ichthy_biom[k]-mean(ichthy_biom[]))+beta5.willAge5*(cope_rich[k]-mean(cope_rich[]))
      ### age return predictions on the arithmetic scale
    ...
    pred_willTotal <- pred_willAge3[nObs.willAge3] + pred_willAge4[nObs.willAge4] + pred_willAge5[nObs.willAge5] + pred_willAge6
    pred_willAdult <- pred_willAge4[nObs.willAge4] + pred_willAge5[nObs.willAge5] + pred_willAge6
    
    ## hatchery projections
    predHat_willAge3 <- pred_willAge3[nObs.willAge3]-(pred_willAge3[nObs.willAge3]*EY[nHWobs])
    predHat_willAge4 <- pred_willAge4[nObs.willAge4]-(pred_willAge4[nObs.willAge4]*EY[nHWobs])
    predHat_willAge5 <- pred_willAge5[nObs.willAge5]-(pred_willAge5[nObs.willAge5]*EY[nHWobs])
    predHat_willAge6 <- pred_willAge6-(pred_willAge6*EY[nHWobs])
    
    predHat_willTotal <- predHat_willAge3 + predHat_willAge4 + predHat_willAge5 + predHat_willAge6
    predHat_willAdult <- predHat_willAge4 + predHat_willAge5 + predHat_willAge6
  }',
  file={willComb.mod <- tempfile()})

## define parameters to monitor
age3nObs <- length(willAge3Fit.dat$willAge2_ret)+1
age4nObs <- length(willAge4Fit.dat$willAge3_col)+1
age5nObs <- length(willAge5Fit.dat$willAge5_col)+1
age6nObs <- length(willAge6Fit.dat)
HWobs <- length(head(willChsHWprop.dat, -1)[,4])+1
...
pred.mcmc.willAge3 <- mcmc.willComb[, paste("pred_willAge3[",age3nObs,"]",sep = "")]
pred.mcmc.willAge4 <- mcmc.willComb[, paste("pred_willAge4[",age4nObs,"]",sep = "")]
pred.mcmc.willAge5 <- mcmc.willComb[, paste("pred_willAge5[",age5nObs,"]",sep = "")]
...
predHat.mcmc.willClpRt <- mcmc.willComb[, paste("EY[",HWobs,"]",sep = "")]
...
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(willAge3Mod.name,
                                                 round(mean(pred.mcmc.willAge3),0),
                                                 round(hdi(pred.mcmc.willAge3, credMass=0.95)[1],0),
                                                 round(hdi(pred.mcmc.willAge3, credMass=0.95)[2],0))

### append output for age 4 to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(willAge4Mod.name,
                                                 round(mean(pred.mcmc.willAge4),0),
                                                 round(hdi(pred.mcmc.willAge4, credMass=0.95)[1],0),
                                                 round(hdi(pred.mcmc.willAge4, credMass=0.95)[2],0))

### append output for age 5 to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(willAge5Mod.name,
...
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(willAge6Mod.name,
                                                 round(mean(pred.mcmc.willAge6),0),
                                                 round(hdi(pred.mcmc.willAge6, credMass=0.95)[1],0),
                                                 round(hdi(pred.mcmc.willAge6, credMass=0.95)[2],0))

### append output for adults to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(willAdultMod.name,
                                                 round(mean(pred.mcmc.willAdult),0),
                                                 round(hdi(pred.mcmc.willAdult, credMass=0.95)[1],0),
                                                 round(hdi(pred.mcmc.willAdult, credMass=0.95)[2],0))

### append output for total to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(willTotalMod.name,
                                                 round(mean(pred.mcmc.willTotal),0),
                                                 round(hdi(pred.mcmc.willTotal, credMass=0.95)[1],0),
                                                 round(hdi(pred.mcmc.willTotal, credMass=0.95)[2],0))

### append output for age 3 hatchery to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(paste(willAge3Mod.name,"Hatchery"),
                                                 round(mean(predHat.mcmc.willAge3),0),
                                                 round(hdi(predHat.mcmc.willAge3, credMass=0.95)[1],0),
                                                 round(hdi(predHat.mcmc.willAge3, credMass=0.95)[2],0))

### append output for age 4 hatchery to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(paste(willAge4Mod.name,"Hatchery"),
                                                 round(mean(predHat.mcmc.willAge4),0),
                                                 round(hdi(predHat.mcmc.willAge4, credMass=0.95)[1],0),
                                                 round(hdi(predHat.mcmc.willAge4, credMass=0.95)[2],0))

### append output for age 5 hatchery to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(paste(willAge5Mod.name,"Hatchery"),
...
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(paste(willAge6Mod.name,"Hatchery"),
                                                 round(mean(predHat.mcmc.willAge6),0),
                                                 round(hdi(predHat.mcmc.willAge6, credMass=0.95)[1],0),
                                                 round(hdi(predHat.mcmc.willAge6, credMass=0.95)[2],0))

### append output for adult hatchery to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(paste(willAdultMod.name,"Hatchery"),
                                                 round(mean(predHat.mcmc.willAdult),0),
                                                 round(hdi(predHat.mcmc.willAdult, credMass=0.95)[1],0),
                                                 round(hdi(predHat.mcmc.willAdult, credMass=0.95)[2],0))

### append output for total hatchery to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c(paste(willTotalMod.name,"Hatchery"),
                                                 round(mean(predHat.mcmc.willTotal),0),
                                                 round(hdi(predHat.mcmc.willTotal, credMass=0.95)[1],0),
                                                 round(hdi(predHat.mcmc.willTotal, credMass=0.95)[2],0))

### append output for predicted clip rate to new data frame (from above)
willComb.pred.out[nrow(willComb.pred.out)+1,]<-c("Clip Rate",
                                                 round(mean(predHat.mcmc.willClpRt),4),
                                                 round(hdi(predHat.mcmc.willClpRt, credMass=0.95)[1],4),
                                                 round(hdi(predHat.mcmc.willClpRt, credMass=0.95)[2],4))
