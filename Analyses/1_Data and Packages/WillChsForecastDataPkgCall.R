...
willChsRet.dat <- read.xlsx(inPath,
                            sheet = 1,
                            colNames = TRUE)

## Willamette covariate data
willChsCov.dat <- read.xlsx(inPath,
                            sheet = 2,
                            colNames = TRUE)

## Willamette HW proportion data
willChsHWprop.dat <- read.xlsx(inPath,
                               sheet = 3,
                               colNames = TRUE)

## Clackamas return data
### call raw data
clackChsRet.dat <- read.xlsx(inPath,
                             sheet = 4,
                             colNames = TRUE)
...
clackChsManip.dat <- cbind(clackChsRet.dat,clackChsTot.dat)
