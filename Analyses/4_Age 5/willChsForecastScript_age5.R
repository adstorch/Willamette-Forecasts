...
willAge5Fit.dat <- na.omit(
  head(
    data.frame(
      willAge5_col = willChsRet.dat$age5_col,
...
## default is to let jags estimate initial values; otherwise use jags() and 
## specify initial values as below)
...
pred.mcmc.willAge5 <- mcmc.willAge5[, paste(
  "pred_willAge5","[",willAge5Mod.dat$nObs,"]",
  sep = ""
)]
